...
const MyEO = () => {
	...
	const onSelect = (cardDetails) => {
		switch (cardDetails.intent) {
			case '+': {
				const itemExists = cartValues.find((item) => item.name === cardDetails.payLoad.name);

				if (itemExists) {
					setCartValues((cart) =>
						cart.map((item) =>
							item.name === cardDetails.payLoad.name ? { ...item, count: item.count + 1 } : item
						)
					);
				} else {
					setCartValues((cart) => [
			...
			case '-': {
				...
				if (itemExists && itemExists.count > 1) {
					...
					setCartValues((cart) => cart.filter((item) => item.name !== cardDetails.payLoad.name));
				}
				break;
			}
		}
	...
	useEffect(() => {
		if (cartValues.length > 0) {
			setBill(cartValues.reduce((acc, item) => acc + +item.count * +item.registrationFee, 0));
